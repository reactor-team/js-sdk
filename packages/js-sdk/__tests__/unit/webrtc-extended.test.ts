...
import {
  ...
  createDataChannel,
  createOffer,
  transformOfferSdpHevcDynamicPts,
  createAnswer,
...
describe("transformOfferSdpHevcDynamicPts()", () => {
  it("strips non-HEVC video formats when H265 rtpmap is present", () => {
    const sdp = [
      "v=0",
    ...
    const out = transformOfferSdpHevcDynamicPts(sdp);
    expect(out).toContain("m=video 9 UDP/TLS/RTP/SAVPF 99");
    expect(out).not.toMatch(/^a=rtpmap:100 /m);
  ...
  it("keeps every HEVC option when the offer lists multiple H265 payload types", () => {
    const sdp = [
      "v=0",
    ...
    const out = transformOfferSdpHevcDynamicPts(sdp);
    expect(out).toMatch(/m=video 9 UDP\/TLS\/RTP\/SAVPF 101 102\r/);

...
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`fast-h3/`](./fast-h3)                         | [`@reactor-models/fast-h3`](https://www.npmjs.com/package/@reactor-models/fast-h3)                          | Multi-scene **episode composer** on the fast-h3 clip queue. Pick a scene count, write scenes with an optional server-side AI writer (BYOK) or by hand, and queue them **chained** (`continue_from_clip_id`) so the episode plays as one continuous video. Live queue mirror, lazy connect, capacity-gated enqueues, snap-clip download.                                               |
| [`fast-h3-livestream/`](./fast-h3-livestream)   | [`reactor-sdk`](https://pypi.org/project/reactor-sdk/) (Python streamer) + LiveKit viewer                   | The **broadcast-channel** shape of fast-h3: a Python streamer drives the clip queue (scene-chained episodes, BYOK prompt upsampling, default-scene rotation) into a LiveKit room; a Next.js viewer plays the room and pitches episode ideas over its chat. Capacity and render errors answered in chat.                                                                               |
...
| [`visko-orbis-stable/`](./visko-orbis-stable)   | [`@reactor-models/visko-orbis-stable`](https://www.npmjs.com/package/@reactor-models/visko-orbis-stable)    | Continuous **steerable video generation** (Visko Orbis, Stable tier). Text-to-video or image-anchored start, **per-chunk live prompt morphing** mid-run, resolution-from-`available_resolutions` + seed knobs, 48 kHz audio with a "generate sound" toggle, snap-clip recording.                                                                                                      |
| [`visko-orbis-dynamic/`](./visko-orbis-dynamic) | [`@reactor-models/visko-orbis-dynamic`](https://www.npmjs.com/package/@reactor-models/visko-orbis-dynamic)  | Continuous **steerable video generation** (Visko Orbis, Dynamic tier). Same T2V / I2V / mid-run morph surface as stable: resolution picker driven by `state.available_resolutions` (includes `native`), a generate-sound toggle via `set_audio_enabled`, and seed — all apply at the next `start`.                                                                                    |

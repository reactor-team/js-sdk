...
| [`lingbot-v2/`](./lingbot-v2)         | `@reactor-models/lingbot-v2` (pre-release snapshot vendored in `app/sdk/` until the package publishes)         | **Lingbot 2** public demo. Two-axis WASD driving (`set_move_longitudinal` + `set_move_lateral`, diagonals included), curated `set_camera_pose` presets (orbit, push-in, crane), dynamic world events as live prompt swaps, `chunk N / total` run progress, snap-clip recording.    |
...
npx create-reactor-app my-app --model lingbot-v2

...
//
// RESOLUTION: rendered ENTIRELY from `snapshot.available_resolutions`.
// The deployment offers a named tier menu (SparkVSR delivers e.g.
// 1080p / 2k / 4k), and the valid strings are whatever that deployment
// was configured with — this example never hard-codes the list, so it
// keeps working if the deployment's menu changes. The pick applies at
// the NEXT start (a running generation keeps the resolution it started
// with, so the track's geometry never jumps mid-shot).
//
// One nuance the UI is honest about: these are DELIVERY (upscaler)
// tiers. The model generates at its own internal size (832×480) and the
// SR stage delivers the picked tier — picking 4k doesn't change what the
// model "dreams", only the output raster.
//
...
export function SessionOptions() {
  const { status, setResolution, setSeed } = useViskoOrbisDistilled();
  ...
  const resolutions: string[] = Array.isArray(snapshot?.available_resolutions)
    ...
    <div className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-3">
      ...
      {resolutions.length > 0 && (
        <>
          ...
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            {resolutions.map((r) => {
              ...
              return (
                <button
                  key={r}
                  onClick={() => setResolution({ resolution: r })}
          ...
          <p className="mt-1.5 text-[10px] leading-snug text-zinc-600">
            Upscaler tier — the model generates at 832×480 and this is the
            delivered raster.
          </p>
        </>
      )}
      ...
      <div className="mt-1.5 flex gap-1.5">
        ...
        <button
          ...
          onClick={() => setSeed({ seed: parseInt(seedText, 10) })}

...
| [`app/api/reactor/token/route.ts`](app/api/reactor/token/route.ts)       | GET route that mints a session-scoped Reactor JWT (pinned to `reactor/helios` via `authorization_details`) and sets `Cache-Control: private, max-age=<token lifetime>`. The browser handles caching transparently — no localStorage, no JWT parsing in client code.                                                                   |

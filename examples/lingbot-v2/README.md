...
- **Start from a curated scene.** Six hand-tuned scenes ship in the sidebar — a tilt-shift miniature cityscape, a macro shot of an ant on a dirt path, a SWAT officer in a noir rain alley, a mounted warrior over a muddy battlefield, a rider gripping a dragon's reins near a castle, a jet-ski rider on a palm beach. Each one bundles a reference image and a paragraph-length prompt that frames both the subject and the camera. One click, and the model begins generating. Below those sit 22 more scenes imported wholesale from the lab's case3 export (`app/lib/case3-scenes.ts`, unpruned for curation), each carrying its own scene-specific event set — from a haunted hospital corridor with number-keyed monsters to a first-person tour of world landmarks.
...
| [`app/Lingbot2App.tsx`](app/Lingbot2App.tsx)                                 | First `"use client"` boundary. Wires `<LingbotWorld2Provider getJwt={fetchToken}>` with a resolver pointing at the cacheable token route (`@reactor-team/js-sdk` ≥ 2.10.1 pattern), lays out the sidebar + video pane.                                                                                                                                                                                           |
...
| [`app/lib/case3-scenes.ts`](app/lib/case3-scenes.ts)                         | GENERATED from the lab's case3 export: 22 scenes (base prompt verbatim + exported first frame) with all 133 lab-authored events — finished rewrites (`finalPrompt`) applied verbatim, raw addenda composed at press time. Unpruned; delete what doesn't hold up.                                                                                                                                             |

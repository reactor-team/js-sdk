...
Two pieces work together: a Next.js GET route that mints (and caches) a session-scoped JWT server-side, and a `getJwt` resolver prop on `<LingbotProvider>` that calls it on every Coordinator HTTP hop.

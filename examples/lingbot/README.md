...
| [`app/api/reactor/token/route.ts`](app/api/reactor/token/route.ts)           | GET route that mints a session-scoped Reactor JWT (pinned to `reactor/lingbot` via `authorization_details`) and sets `Cache-Control: private, max-age=<token lifetime>`. The browser handles caching transparently — no localStorage, no JWT parsing in client code. |
| [`app/LingbotApp.tsx`](app/LingbotApp.tsx)                                   | First `"use client"` boundary. Wires `<LingbotProvider getJwt={fetchToken}>` with a resolver pointing at the cacheable token route (`@reactor-team/js-sdk` ≥ 2.10.1 pattern), lays out the sidebar + video pane.                                                     |

...
> **About this skeleton.** This is the Lingbot 2 public-demo skeleton. The typed `@reactor-models/lingbot-2` package is not published yet (blocked on the typed-SDK codegen, Linear REA-3595), so the app currently imports the published `@reactor-models/lingbot` package and rebrands the surface as Lingbot 2. Everything below is written against that shared SDK surface; the symbol names (`useLingbot`, `LingbotProvider`, `LingbotMainVideoView`, the `set_*` commands) stay the same when you swap to the v2 package, so the patterns carry over unchanged.

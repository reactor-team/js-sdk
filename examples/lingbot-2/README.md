...
> **Skeleton status.** This is the public-demo skeleton for Lingbot 2. Its typed SDK, `@reactor-models/lingbot-2`, is not published yet (blocked on the typed-SDK codegen, Linear REA-3595). Until it ships, the skeleton imports the published [`@reactor-models/lingbot`](https://www.npmjs.com/package/@reactor-models/lingbot) package so it installs, builds, and runs today. When the v2 package lands, swap the dependency in `package.json` and update every `@reactor-models/lingbot` import across `app/` ([`app/Lingbot2App.tsx`](app/Lingbot2App.tsx) plus the sidebar components); the hook and provider surface is intended to be drop-in compatible.
...
> **Start a standalone project:** `npx create-reactor-app my-app --model=lingbot-2` scaffolds this example into a fresh app — no clone needed. The steps below are for running it in-place from a monorepo checkout.
...
| [`app/Lingbot2App.tsx`](app/Lingbot2App.tsx)                                 | First `"use client"` boundary. Wires `<LingbotProvider getJwt={fetchToken}>` with a resolver pointing at the cacheable token route (`@reactor-team/js-sdk` ≥ 2.10.1 pattern), lays out the sidebar + video pane.    |
